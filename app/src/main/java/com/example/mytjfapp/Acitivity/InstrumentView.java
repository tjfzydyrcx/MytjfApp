...
//此时求出来的baseline可以使文字竖直居中
    ...
    private Region pointerRegion1 = new Region();  //指针区域
...
//        path.computeBounds(rectF, true);
        pointerRegion.setPath(path, new Region((int) rectF.left, (int) rectF.top, (int) rectF.right, (int) rectF.bottom));
        ...
        path.close();
        pointerRegion1.setPath(path, new Region((int) rectF.left, (int) rectF.top, (int) rectF.right, (int) rectF.bottom));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            startx = event.getX();
            starty = event.getY();
            if (pointerRegion.contains((int) startx, (int) starty)||pointerRegion1.contains((int) startx, (int) starty)) {  //在其中
                isChoosePointer = true;
                LogUtils.e("zoule=111==");
                invalidate();
            ...
            float x = event.getX();
            float y = event.getY();
            LogUtils.e(x + "zoule=222==" + y);

            LogUtils.e(centerPoint[0] + "zoule=iiiiii==" + centerPoint[1]);

            if (isChoosePointer) {
                if (y <= getMeasuredHeight() && x != getMeasuredWidth() / 2) {
                    double degree = Math.atan2((getMeasuredHeight() - y), (getMeasuredWidth() / 2 - x));
                    setProgress((int) (degree / Math.PI * 180));
                } else {
                    if (y > getMeasuredHeight() && x < getMeasuredWidth() / 2) {  //说明滑到下面了
                        setProgress(0);
                    } else if (y > getMeasuredHeight() && x > getMeasuredWidth() / 2) {
                        setProgress(180);
                    }
                }
